...
namespace ComputerExam.Util
{
    public class FtpWeb
    {
        ...
        /// <param name="filePath"></param> 
        /// <param name="fileName"></param> 
        public void Download(string filePath, string fileName, string Uri, ToolStripProgressBar prog, ToolStripStatusLabel label1, string message)
        {
            FtpWebRequest reqFTP;
            ...
            try
            {
                downFileName = filePath + Path.DirectorySeparatorChar + fileName;
                if (!Directory.Exists(filePath)) Directory.CreateDirectory(filePath);
                outputStream = new FileStream(downFileName, FileMode.Create);
                string Url = ftpURI;
                if (!string.IsNullOrEmpty(Uri)) Url = string.Format("{0}//{1}//", Url, Uri);
                reqFTP = (FtpWebRequest)FtpWebRequest.Create(new Uri(Url + fileName));
                reqFTP.Method = WebRequestMethods.Ftp.DownloadFile;
                reqFTP.UseBinary = true;
                reqFTP.Timeout = timeOut;
                ...
                response = (FtpWebResponse)reqFTP.GetResponse();
                ftpStream = response.GetResponseStream();
                totalBytes = GetFileSize(reqFTP.RequestUri.OriginalString);
            ...
            {
                throw new WebException("FTP下载文件过程中发生错误：" + we.Message);
            }
            catch (Exception ex)
            {
                if (File.Exists(downFileName)) File.Delete(downFileName);
                throw new Exception("FTP下载文件过程中发生错误：" + ex.Message);
            }
            finally
            {
