...
namespace ComputerExam.Util
{
    public class PublicClass
    {
        ...
        public static JobDataHandler rjdh = new JobDataHandler();
        /// <summary>
        /// 1：试卷评分，2：交卷退出
        /// </summary>
        ...
        public M_PaperInfo GetStudentPaperInfo(string StudentDir, ref string ErrorInfo)
        {
            ...
            List<M_SubTopic> aSubTopicList = new List<M_SubTopic>();
            string sPaperInfo = string.Empty;
            string sTopicList = string.Empty;
            string sSubTopicList = string.Empty;
            ...
            List<string> slGUID = new List<string>();
            ...
            List<string> slSubTopicID = new List<string>();
            List<string> slSubTopicFaceID = new List<string>();
            List<string> slSubOptionNumber = new List<string>();
            List<string> slSubTimeLimit = new List<string>();
            List<string> slSubSelfTimeLimit = new List<string>();
            List<string> slSubReadingText = new List<string>();
            List<string> slSubIsReading = new List<string>();
            List<string> slSubMediaPath = new List<string>();
            List<string> slSubPlayTimes = new List<string>();
            List<string> slSubRecordPath = new List<string>();
            List<string> slSubRecordDuration = new List<string>();
            List<string> slSubMediaType = new List<string>();
            List<string> slSubMediaBeginTime = new List<string>();
            List<string> slSubRecordBeginTime = new List<string>();
            ...
            slPeiWuTopic = xml.SplitString(xml.GetXmlNodeValue(sPaperInfo, "配伍题"), xml.SplitFlagStr);
            #endregion
            ...
            for (int i = 0; i < iTopicTypeNumber; i++)
            {
                ...
                tixing.AllowPeiWu = (slPeiWuTopic[i] == "1");
                ...
                slSubTopicCount = xml.SplitString(xml.GetXmlNodeValue(sTopicList, "小题数"), xml.SplitFlagStr);
                ...
                for (int j = 0; j < iTopicNumber; j++)
                {
                    ...
                    shitixinxi.AllowSubTopic = aTopicTypeList[i].AllowSubTopic;
                    shitixinxi.AllowPeiWu = aTopicTypeList[i].AllowPeiWu;
                    shitixinxi.TopicId = slTopicId[j];
                    shitixinxi.GUID = slGUID[j];
                    shitixinxi.SubTopicId = slSubTopicId[j];
                    shitixinxi.SubTopicCount = int.Parse(slSubTopicCount[j]);
                    ...
                    shitixinxi.DifficultyID = int.Parse(slDifficultyID[j]);
                    shitixinxi.DifficultyName = slDifficultyName[j];
                    ...
                    #region 初始化子试题信息
                    if (shitixinxi.AllowSubTopic)
                    {
                        sSubTopicList = SowerExamPlugn.GetSubTopicList(StudentDir, int.Parse(shitixinxi.TopicTypeId), int.Parse(shitixinxi.TopicId));
                        slSubTopicID = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "ID"), xml.SplitFlagStr);
                        slSubTopicFaceID = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "题面ID"), xml.SplitFlagStr);
                        slSubOptionNumber = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "选项个数"), xml.SplitFlagStr);
                        slSubTimeLimit = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "TimeLimit"), xml.SplitFlagStr);
                        slSubSelfTimeLimit = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "SelfTimeLimit"), xml.SplitFlagStr);
                        slSubReadingText = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "ReadingText"), xml.SplitFlagStr);
                        slSubIsReading = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "IsReading"), xml.SplitFlagStr);
                        slSubMediaPath = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "MediaPath"), xml.SplitFlagStr);
                        slSubPlayTimes = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "PlayTimes"), xml.SplitFlagStr);
                        slSubRecordPath = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "RecordPath"), xml.SplitFlagStr);
                        slSubRecordDuration = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "RecordDuration"), xml.SplitFlagStr);
                        slSubMediaType = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "MediaType"), xml.SplitFlagStr);
                        slSubMediaBeginTime = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "MediaBeginTime"), xml.SplitFlagStr);
                        slSubRecordBeginTime = xml.SplitString(xml.GetXmlNodeValue(sSubTopicList, "RecordBeginTime"), xml.SplitFlagStr);

                        iSubTopicNumber = slSubTopicID.Count;
                        if (iSubTopicNumber != slSubTopicID.Count || iSubTopicNumber != slSubTopicFaceID.Count ||
                           iSubTopicNumber != slSubOptionNumber.Count || iSubTopicNumber != slSubTimeLimit.Count ||
                           iSubTopicNumber != slSubSelfTimeLimit.Count || iSubTopicNumber != slSubReadingText.Count)
                        {
                            ErrorInfo = "获取的试题数据中的数组元素最大值不匹配";
                            return aPaperInfo;
                        }
                        aSubTopicList = new List<M_SubTopic>();
                        for (int k = 0; k < iSubTopicNumber; k++)
                        {
                            M_SubTopic subTopic = new M_SubTopic();
                            subTopic.TopicID = shitixinxi.TopicId;
                            subTopic.TopicTypeID = shitixinxi.TopicTypeId;
                            subTopic.SubTopicID = slSubTopicID[k];
                            subTopic.TopicFaceID = slSubTopicFaceID[k];
                            subTopic.OptionNumber = int.Parse(slSubOptionNumber[k]);
                            subTopic.TimeLimit = slSubTimeLimit[k];
                            subTopic.SelfTimeLimit = slSubSelfTimeLimit[k];
                            subTopic.ReadingText = slSubReadingText[k];
                            subTopic.IsReading = slSubIsReading[k];
                            subTopic.MediaPath = slSubMediaPath[k];
                            subTopic.PlayTimes = slSubPlayTimes[k];
                            subTopic.RecordPath = slSubRecordPath[k];
                            subTopic.RecordDuration = slSubRecordDuration[k];
                            subTopic.MediaType = slSubMediaType[k];
                            subTopic.MediaBeginTime = slSubMediaBeginTime[k];
                            subTopic.RecordBeginTime = slSubRecordBeginTime[k];
                            aSubTopicList.Add(subTopic);
                        }
                        shitixinxi.SubTopics = aSubTopicList;
                    }
                    #endregion
        ...
        /// <summary>
        /// 试题做对级别(0:没有做对 1:半对 2:全对)
        /// </summary>
        /// <param name="topicPoint">试题分值</param>
        /// <param name="topicScore">试题得分</param>
        /// <returns></returns>
        public int GetTopicRightLevel(double topicPoint, double topicScore)
        {
            var level = 0;
        ...
        public string ReturnRequest(string param, string body, string code)
        {
            string Msg = "";
            string sResult = "";
            string RXml = "";

            RXml = ReturnRequest("1.0", 1, DateTime.Now.ToString("yyyy-MM-dd"), sResult, code, param, body, ref Msg);
            return RXml;
        }
        ...
        public string ReturnRequest(string ver, int state, string workdate, string exsm, string excode, string param, string body, ref string Msg)
        {
            try
            {
                ...
                string MD5Text = sb.ToString() + "sowerpower@Sower.com";
        ...
        public bool IsRight(string xml)
        {
            ...
            string Result = FormsAuthentication.HashPasswordForStoringInConfigFile(validTxt + "sowerpower@Sower.com", "MD5");//asdfasweroojj
        ...
        {
            string strUrl = string.Format("http://{0}/{1}/AutoUpdaterList.xml",
                UserConfigSettings.Instance.ReadSetting("更新地址"), UserConfigSettings.Instance.ReadSetting("更新目录"));     //获得IP的网址
            Uri uri = new Uri(strUrl);
            WebRequest webreq = WebRequest.Create(uri);
            Stream s = webreq.GetResponse().GetResponseStream();
        ...
        {
            string strPath = Application.StartupPath + "\\AutoUpdaterList.xml";

            if (File.Exists(strPath))
            {
                StreamReader streamReader = new StreamReader(strPath, Encoding.UTF8);
        ...
        public static string GetStudentDir()
        {
            ...
            int sysDirLength = Environment.SystemDirectory.Length;
            string sysDisk = Environment.SystemDirectory.Remove(3, sysDirLength - 3);
            drives.Remove(sysDisk);
            return drives[0];
        }
    }
}
